...
<nav aria-label="breadcrumb" style="margin: 80px auto; width: 900px; font-size: 20px;">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">สเต็กและซุป</li>
    </ol>
    <?php
       
       if(isset($_POST['valueToSearch'])){
           $valueToSearch = $_POST['valueToSearch'];
           $query  = "SELECT * FROM food WHERE food_type_id = '4'AND calorie  LIKE '%$valueToSearch%'";
           $search_result = filterTable($query);
       }else{
           $query = "SELECT * FROM food WHERE food_type_id = '4'";
           $search_result = filterTable($query);
       }
       function filterTable($query)
{
   ...
   ?>
   <form class="d-flex" action="food4.php" method="post">
