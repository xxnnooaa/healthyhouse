...
<nav aria-label="breadcrumb" style="margin: 80px auto; width: 900px; font-size: 20px;">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">ผัดและต้ม</li>
    </ol>
    <?php
       
       if(isset($_POST['valueToSearch'])){
           $valueToSearch = $_POST['valueToSearch'];
           $query  = "SELECT * FROM food WHERE food_type_id = '5' AND calorie  LIKE '%$valueToSearch%'";
           $search_result = filterTable($query);
       }else{
           $query = "SELECT * FROM food WHERE food_type_id = '5'";
           $search_result = filterTable($query);
       }
       function filterTable($query)
{
   ...
   ?>
   <form class="d-flex" action="food5.php" method="post">
